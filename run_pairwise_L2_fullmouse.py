...
"""def run_pairwise(curves, graphtype):
    ...
    for dim in [0,1,2]:
        ...
        save_distance_heatmap(distance_matrices[dim], names, dim, 'PairwiseDistances/Full/'+graphtype+'_Pressure'+pressure+'_Dim'+str(dim), title=f"{graphtype} Curve Pairwise $L^2$ Distances (Dimension {dim}, Full Mouse)", 
        ...
        csv_filename = 'PairwiseDistances/Full/'+graphtype+'_Pressure'+pressure+'_Dim'+str(dim)+'.csv'
...
def run_pairwise(curves, graphtype, pressure, metrics_dict):
    keys = list(curves.keys())
    curve_list = [curves[k] for k in keys]
    
    group1_indices = list(range(5))
    group2_indices = list(range(5, 13))
    ...
    for dim in [0,1,2]:
        save_distance_heatmap(distance_matrices[dim], names, dim, 
            ...
            title=f"{graphtype} Curve Pairwise $L^2$ Distances (Dimension {dim}, Full Mouse)", 
            figsize=(12, 8), cmap='viridis', annot=True, fmt='.3f', dpi=300,
    ...
    # Compute metrics for each dataset and add to metrics_dict
    for name, curve_dict in curves.items():
        metrics = compute_curve_metrics(curve_dict, dims=[0, 1, 2])
        
        # Create key for this dataset-pressure combination
        key = (name, pressure)
        if key not in metrics_dict:
            metrics_dict[key] = {'dataset': name, 'pressure': pressure}
        
        # Add metrics with curve type suffix
        for metric_name, metric_value in metrics.items():
            metrics_dict[key][f'{metric_name}_{graphtype}'] = metric_value
...
for pressure in ['1','2','3','4']:
    ...
    for name, diag in persistence_diagrams.items():
        ...
        lifespan_curves[name] = compute_lifespan_curves(diag, t_min = 0, 
            t_max=global_last_deaths[max(global_last_deaths.keys())], resolution = 100000)

    betti_curves = {}
    for name, diag in persistence_diagrams.items():
        points = datasets[name]
        betti_curves[name] = compute_betti_curve(diag, 
            cutoff=global_last_deaths[max(global_last_deaths.keys())])

    norm_lifespan_curves = {}
    for name, diag in persistence_diagrams.items():
        points = datasets[name]
        norm_lifespan_curves[name] = compute_norm_lifespan_curves(diag, t_min = 0, 
            t_max=global_last_deaths[max(global_last_deaths.keys())], resolution = 100000)

    run_pairwise(lifespan_curves, 'Lifespan', pressure, all_metrics)
    run_pairwise(betti_curves, 'Betti', pressure, all_metrics)
    run_pairwise(norm_lifespan_curves, 'Norm_Lifespan', pressure, all_metrics)
...
# Sort columns: first dataset and pressure, then all metrics alphabetically for consistency
...
metrics_df.to_csv('PairwiseDistances/Full/curve_metrics_summary_full.csv', index=False)
print(f"\nSaved curve metrics summary to PairwiseDistances/Full/curve_metrics_summary.csv")
